...
export default {
    ...
    mounted() {
        const storedPrimaryColor = localStorage.getItem('selectedPrimaryColor');
        const storedSurfaceColor = localStorage.getItem('selectedSurfaceColor');


        if (storedPrimaryColor) {
            const primaryColor = this.primaryColors.find(c => c.name === storedPrimaryColor);
            if (primaryColor) {
                this.selectedPrimaryColor = primaryColor.name;
                this.applyTheme('primary', primaryColor);
            }
        }

        if (storedSurfaceColor) {
            const surfaceColor = this.surfaces.find(s => s.name === storedSurfaceColor);
            if (surfaceColor) {
                this.selectedSurfaceColor = surfaceColor.name;
